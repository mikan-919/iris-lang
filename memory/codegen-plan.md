...
- **トレイト**を追加（ADR-0004〜0009、`tests/traits.rs`）: `impl Trait for Type` のメソッドは固有メソッドと同じく `@Type.method` を emit。**既定実装**は未上書きの impl ごとに `Self→実装型` の置換で本体を合成して emit。**ジェネリック関数の単相化**: ジェネリックテンプレートは出さず、`type_info.mono`（callee span→型引数）を種に、各本体の呼び出しを walk して必要な実体を集め、ワークリストで閉包（入れ子も）を処理し `@f.Type`（`mono_symbol`/`mangle_ty`）を emit。FnCodegen に `type_subst`（型パラメータ→具体型）を持たせ、`lower()`/`raw_ty()` で全型導出に適用（`subst_ty`）。**記号衝突**（固有 vs trait・複数 trait の同名）は `method_defs`(型,メソッド,提供元ラベル)→Function と `method_collisions` で検出し、衝突時のみ trait を `@Type.Trait.method` に分離。dispatch は `#`修飾子 or typeck 記録の `method_provider`(span→ラベル) で提供元を選ぶ。スーパートレイト由来メソッドはラベル＝定義トレイト名。**構造的 `==`/`!=`**（ADR-0009）: `gen_struct_eq`/`gen_eq_at` がフィールドを再帰比較し AND 合成、`!=` は xor。
- 未対応（次段）: enum（バリアント構築/分解は frontend も未対応）、`!`、文字列の操作（長さ・索引・連結・補間）、ジェネリック**型**の単相化（関数は実装済み）、`Eq`/順序/算術トレイト、`Type.func()`（self なし関連関数）。WASM、JIT。
