...
- 固有メソッド `impl Type { fn m(self / &self / &mut self, ...): Ret { ... } }`（トレイト無し）。
  ...
  trait 定義・`impl..for`・適合検査・デフォルト実装・関連関数の呼び出し構文は未対応
...
- `trait` 定義 / `impl Trait for Type`（適合検査・デフォルト実装）。固有メソッド `impl Type { ... }`
  は実装済み（上記）。trait システム本体は未実装だが、設計は ADR-0004〜0009 で確定:
  - **ディスパッチは静的のみ**・`dyn` なし・単相化ジェネリクス（ADR-0006）。境界型
    `x: Greet + Serialize` は引数位置限定の匿名ジェネリクス糖衣。
  - **ジェネリックトレイト**（`trait Iterator<T>`）・**関連型なし**・複数 instantiation 可・
    **スーパートレイト** `trait Sub: Super`（ADR-0007）。
  - **名前衝突は呼び出し側 `#` で明示**: `x.m#Trait<Args>()` / `for x#i32 in c`。固有 vs trait の
    同名も曖昧扱い（ADR-0004）。
  - **コヒーレンスは `use` スコープ内で一意**（オーファン規則なし、ADR-0008）。
  - **`Self` キーワード・self なし関連関数**を `型名.func()` / `T.func()` で呼ぶ。
  - **ユーザ型は move-only**・暗黙 Copy なし・複製は明示 `clone`（ADR-0005）。
  - **`==`/`!=` は struct で構造的フィールド比較が既定**・`Eq` で上書き、順序/算術は明示トレイト
    のみ（ADR-0009）。
  - 現状は固有メソッドの**同型同名**二重定義のみ検出（`#` 修飾子・trait 本体は未実装）
