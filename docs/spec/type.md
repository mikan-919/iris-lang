...
配列リテラル `[...]` は型注釈に応じて固定長配列 `T[]` か動的配列 `Vec<T>` を構築する（型指向。
注釈がなければ既定で `T[]`）。固定長配列はスタックに、`Vec<T>` はヒープ（`malloc`）に確保される。
いずれも `for x in coll` で要素を反復できる（`control.md` 参照。当面は要素が Copy 型のときのみ）。
構築・反復は実装済み。`push` / 長さ取得 / 索引 `v[i]` / スライスは未実装。配列とタプル `(A, B)` は別物。
