...
fn resolves_params_and_locals() {
    ...
    let res = resolve(&program, Default::default()).expect("名前解決に成功するはず");
...
fn allows_mutual_recursion() {
    ...
    resolve(&program, Default::default()).expect("相互参照できるはず");
}

#[test]
fn reports_undefined_name() {
    let program = parse_src("fn f() {\n    return y\n}");
    let errs = resolve(&program, Default::default()).expect_err("未定義の名前でエラーになるはず");
...
fn reports_duplicate_function() {
    let program = parse_src("fn f() {\n}\nfn f() {\n}");
    let errs = resolve(&program, Default::default()).expect_err("関数名重複でエラーになるはず");
...
fn reports_duplicate_param() {
    ...
    let errs = resolve(&program, Default::default()).expect_err("引数名重複でエラーになるはず");
...
fn allows_local_shadowing() {
    ...
    let program = parse_src("fn f() {\n    let x = 1\n    let x = 2\n    return x\n}");
    resolve(&program, Default::default()).expect("シャドーイングは許可されるはず");
}
...
fn local_out_of_scope_after_block() {
    ...
    let errs = resolve(&program, Default::default()).expect_err("ブロック外で inner は未定義のはず");
...
fn collects_multiple_errors() {
    let program = parse_src("fn f() {\n    return a + b\n}");
    let errs = resolve(&program, Default::default()).expect_err("複数の未定義エラーになるはず");
...
fn def_kinds_are_recorded() {
    ...
    let res = resolve(&program, Default::default()).expect("成功するはず");
