...
| 051 | [iris-lsp: LSP サーバー + Zed インテグレーション](051-iris-lsp-zed.md) | tooling | M | LOW | なし | DONE/MERGED（`feat/first/nom-0`。`LspDiagnostic`+`check_diagnostics` を `src/lib.rs` に追加。`iris-lsp` クレートを新設しワークスペース化。`cargo install` 済み・Zed `settings.json` に登録） |
| 052 | [tree-sitter グラマー + Zed ローカル拡張でシンタックスハイライト](052-tree-sitter-zed-highlight.md) | tooling | M | LOW | なし | DONE（worktree-agent-afccc0985b52e6190 / branch `worktree-agent-afccc0985b52e6190`、未マージ。grammar.js に GLR conflict 宣言 7 件追加。parse ERROR ゼロ・全 159 テスト緑。`tree-sitter highlight` は CLI の config.json 設定が要るため手動確認が必要。Zed 登録はユーザ手動） |
